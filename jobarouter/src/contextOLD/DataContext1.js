...
useEffect(() => {
    const fetchDataFromIndexedDB = async () => {
      try {
        ...
        const transaction = db.transaction(['yourObjectStore1'], 'readonly');
        const objectStore = transaction.objectStore('yourObjectStore1');
  ...
  const openDatabase = () => {
    return new Promise((resolve, reject) => {
      const request = indexedDB.open('yourDatabaseName1', 1);

      request.onupgradeneeded = function (event) {
        const db = event.target.result;
        const objectStore = db.createObjectStore('yourObjectStore1', { keyPath: 'id' });
  ...
  const addDataToIndexedDB = async (newData) => {
    try {
      const db = await openDatabase();
      const transaction = db.transaction(['yourObjectStore1'], 'readwrite');
      const objectStore = transaction.objectStore('yourObjectStore1');
  ...
  return (
    <DataContext1.Provider value={{ data, setData, addDataToIndexedDB, fetchDataByCategory }}>
      {children}
...
const useData = () => {
  ...
  return context;//div association?
};

export { DataProvider1, useData };
